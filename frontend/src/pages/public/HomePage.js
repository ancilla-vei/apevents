...
export default function HomePage() {
  ...
  const iconMap = { Photography: <Camera size={28}/>, Catering: <Users size={28}/>, Music: <Music size={28}/>, Decoration: <Star size={28}/>, default: <Calendar size={28}/> };

  const heroBg = settings?.backgroundImage ? `http://localhost:5000${settings.backgroundImage}` : weddingBg;
  ...
  // ✅ Helper: display label for instagram (show handle if URL, else raw value)
  const instagramDisplay = (() => {
    ...
    try {
      ...
      return val; // not a URL, show as-is
    }
  })();
  ...
  return (
    <div>
      ...
      <section style={{ minHeight: '100vh', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, zIndex: 0, backgroundImage: `url(${heroBg})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(3px) brightness(0.5)', transform: 'scale(1.06)' }} />
        ...
        {Array.from({ length: 22 }).map((_, i) => (
          <div key={i} className="hero-spark" style={{ width: i % 4 === 0 ? 6 : i % 3 === 0 ? 4 : 3, height: i % 4 === 0 ? 6 : i % 3 === 0 ? 4 : 3, left: `${6 + (i * 4.2) % 88}%`, top: `${10 + (i * 6.7) % 78}%`, animationDuration: `${2.5 + (i % 5) * 0.6}s`, animationDelay: `${(i % 7) * 0.45}s`, zIndex: 3, boxShadow: '0 0 8px 3px rgba(255,215,0,0.55)' }} />
        ))}
        <div className="hero-content" style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '6rem 1.5rem 2rem', maxWidth: 780, margin: '0 auto', width: '100%' }}>
          ...
          <p style={{ color: 'rgba(255,215,0,0.9)', fontWeight: 700, letterSpacing: '0.35em', textTransform: 'uppercase', fontSize: '0.8rem', marginBottom: '0.6rem', textShadow: '0 1px 8px rgba(0,0,0,0.6)' }}>Welcome to</p>
          <h1 className="gold-shimmer" style={{ fontSize: 'clamp(3.2rem, 10vw, 6.5rem)', fontFamily: 'Playfair Display', fontWeight: 700, lineHeight: 1.05, marginBottom: '0.9rem', textShadow: 'none', letterSpacing: '0.02em' }}>
        ...
        <div className="container">
          ...
          {services.length > 0 ? (
            <div className="grid-3">
              {services.map(s => (
                <div key={s._id} className="card service-card" style={{ textAlign: 'center', transition: 'transform 0.25s, box-shadow 0.25s', cursor: 'default' }}>
                  {s.image
                    ? <img src={`http://localhost:5000${s.image}`} alt={s.name} style={{ width: '100%', height: 180, objectFit: 'cover', borderRadius: 8, marginBottom: '1rem' }} />
        ...
        <div className="container">
          ...
          {gallery.length > 0 ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1rem' }}>
              {gallery.map(g => (
                <div key={g._id} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', height: 200 }}>
                  <img src={`http://localhost:5000${g.image}`} alt={g.eventName || 'gallery'}
        ...
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '3rem' }}>
            <div>
              ...
              {/* ✅ Instagram — clickable link that opens in new tab */}
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.25rem', alignItems: 'flex-start' }}>
